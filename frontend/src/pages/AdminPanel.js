...
// MemberCard Component for enhanced member management
function MemberCard({ membership, spaceId, spaceName, onReload, handlers }) {
  const [showActions, setShowActions] = useState(false);
  const [managedSpaces, setManagedSpaces] = useState([]);
  const [loadingSpaces, setLoadingSpaces] = useState(false);

  const loadManagedSpaces = async () => {
    if (membership.role === 'manager' || membership.user?.role === 'admin') {
      setLoadingSpaces(true);
      try {
        const { data } = await usersAPI.getManagedSpaces(membership.user_id);
        setManagedSpaces(data || []);
      } catch (error) {
        console.error('Failed to load managed spaces');
      } finally {
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1">
            ...
            {loadingSpaces && <p className="text-xs mt-1" style={{ color: '#8E8E8E' }}>Loading managed spaces...</p>}
            {!loadingSpaces && managedSpaces.length > 0 && (
              <p className="text-xs mt-1" style={{ color: '#7C3AED' }}>
                Manager of: {managedSpaces.map(s => s.name).join(', ')}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {membership.status === 'blocked' ? (
              <Button
                ...
                size="sm"
                onClick={() => {
                  handlers.handleUnblockMember(spaceId, membership.user_id, membership.user?.name);
                  onReload();
                }}
                className="w-full"
              >
              ...
              <>
                ...
                {membership.user?.role === 'admin' ? (
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      handlers.handleDemoteFromAdmin(membership.user_id, membership.user?.name);
                      onReload();
                    }}
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      handlers.handlePromoteToAdmin(membership.user_id, membership.user?.name);
                      onReload();
                    }}
                    className="w-full border-green-400 text-green-600"
                  >
                ...
                {membership.role === 'manager' ? (
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      handlers.handleDemoteFromManager(spaceId, membership.user_id, membership.user?.name);
                      onReload();
                    }}
                    className="w-full border-purple-400 text-purple-600"
                  >
                    Remove as Manager (This Space)
                  </Button>
                ) : membership.user?.role !== 'admin' && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                      handlers.handlePromoteToManager(spaceId, membership.user_id, membership.user?.name);
                      onReload();
                    }}
                    className="w-full border-purple-400 text-purple-600"
                  >
                    Make Manager (This Space)
                  </Button>
                )}

                {/* Add as Manager to Multiple Spaces */}
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handlers.openSpaceSelector(membership.user_id, membership.user?.name)}
                ...
                {membership.user?.role !== 'admin' && (
                  <>
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        handlers.handleBlockMember(spaceId, membership.user_id, membership.user?.name);
                        onReload();
                      }}
                      className="w-full"
                    >
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        handlers.handleRemoveMember(spaceId, membership.user_id, membership.user?.name);
                        onReload();
                      }}
